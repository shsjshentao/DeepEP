...
Buffer::Buffer(int rank,
    ...
    EP_HOST_ASSERT(num_rdma_bytes % NUM_BUFFER_ALIGNMENT_BYTES == 0 and (low_latency_mode or disable_nvlink_for_normal_mode or num_rdma_bytes <= std::numeric_limits<int>::max()));
    ...
    EP_HOST_ASSERT(0 <= rank and rank < num_ranks and (num_ranks <= NUM_MAX_NVL_PEERS * NUM_MAX_RDMA_PEERS or low_latency_mode or disable_nvlink_for_normal_mode));
    EP_HOST_ASSERT(num_ranks < NUM_MAX_NVL_PEERS or num_ranks % NUM_MAX_NVL_PEERS == 0);
    if (num_rdma_bytes > 0)
        EP_HOST_ASSERT(num_ranks > NUM_MAX_NVL_PEERS or low_latency_mode or disable_nvlink_for_normal_mode);
    ...
    EP_HOST_ASSERT(num_rdma_ranks == 1 and not low_latency_mode and not disable_nvlink_for_normal_mode and "NVSHMEM is disabled during compilation");
#endif
    ...
    // lowlatency mode or normal mode w/o nvl  nvl_bytes = 0
    if (num_nvl_bytes > 0) {
...
void Buffer::sync(const std::vector<int> &device_ids,
    ...
    if (num_rdma_bytes > 0) {
        ...
        auto nvshmem_rank = low_latency_mode or disable_nvlink_for_normal_mode ? rank : rdma_rank;
        auto num_nvshmem_ranks = low_latency_mode or disable_nvlink_for_normal_mode ? num_ranks : num_rdma_ranks;
        EP_HOST_ASSERT(nvshmem_rank == internode::init(root_unique_id, nvshmem_rank, num_nvshmem_ranks, low_latency_mode, disable_nvlink_for_normal_mode));
        internode::barrier();
...
std::tuple<torch::Tensor, std::optional<torch::Tensor>, std::optional<torch::Tensor>, std::optional<torch::Tensor>, std::optional<torch::Tensor>, std::vector<int>, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, std::optional<EventHandle>>
Buffer::intranode_dispatch(const torch::Tensor& x, const std::optional<torch::Tensor>& x_scales,
    ...
    // FP8/NVFP4 scales checks
    int num_scales = 0, scale_token_stride = 0, scale_hidden_stride = 0, num_sf_scales_for_nvfp4 = 0, sf_scale_for_nvfp4_token_stride = 0, sf_scale_for_nvfp4_hidden_stride = 0;
    float* x_scales_ptr = nullptr;
    float* sf_scale_for_nvfp4_ptr = nullptr;
    if (x_scales.has_value()) {
        if (use_nvfp4) {
            // NVFP4 scales check
            EP_HOST_ASSERT(x.element_size() == 4); // Uses Int32 for packed NVFP4
            EP_HOST_ASSERT(x_scales->scalar_type() == torch::kInt);
            ...
            // NVFP4 per-token scale check
            EP_HOST_ASSERT(sf_scale_for_nvfp4.has_value() and sf_scale_for_nvfp4->dim() == 2);
            EP_HOST_ASSERT(sf_scale_for_nvfp4->size(0) == num_tokens);
            EP_HOST_ASSERT(sf_scale_for_nvfp4->scalar_type() == torch::kFloat32);
            num_sf_scales_for_nvfp4 = static_cast<int>(sf_scale_for_nvfp4->size(1));
            sf_scale_for_nvfp4_ptr = static_cast<float*>(sf_scale_for_nvfp4->data_ptr());
            sf_scale_for_nvfp4_token_stride = static_cast<int>(sf_scale_for_nvfp4->stride(0));
            sf_scale_for_nvfp4_hidden_stride = static_cast<int>(sf_scale_for_nvfp4->stride(1));
        } else {
            // FP8 scales check
    ...
    auto recv_topk_idx = std::optional<torch::Tensor>(), recv_topk_weights = std::optional<torch::Tensor>(), recv_x_scales = std::optional<torch::Tensor>(), recv_sf_scale_for_nvfp4 = std::optional<torch::Tensor>();
    ...
    float* recv_sf_scale_for_nvfp4_ptr = nullptr;
    if (topk_idx.has_value()) {
    ...
    if (use_nvfp4 and sf_scale_for_nvfp4.has_value()) {
        recv_sf_scale_for_nvfp4 = torch::empty({num_recv_tokens, num_sf_scales_for_nvfp4}, sf_scale_for_nvfp4->options());
        recv_sf_scale_for_nvfp4_ptr = static_cast<float*>(recv_sf_scale_for_nvfp4->data_ptr());
    }
    ...
    EP_HOST_ASSERT(num_ranks * num_ranks * sizeof(int) +                                                                    // Size prefix matrix
                   ...
                   num_channels * num_ranks * config.num_max_nvl_chunked_recv_tokens * sizeof(float) * num_sf_scales_for_nvfp4 // NVFP4 sf_scale buffer
                   <= num_nvl_bytes);
    intranode::dispatch(recv_x.data_ptr(), recv_x_scales_ptr, recv_sf_scale_for_nvfp4_ptr, recv_src_idx.data_ptr<int>(), recv_topk_idx_ptr, recv_topk_weights_ptr, recv_channel_prefix_matrix.data_ptr<int>(),
                        send_head.data_ptr<int>(),
                        x.data_ptr(), x_scales_ptr, sf_scale_for_nvfp4_ptr, topk_idx_ptr, topk_weights_ptr,
                        ...
                        num_topk, num_experts, num_scales, num_sf_scales_for_nvfp4,
    ...
    if (async) {
        ...
        for (auto& to: {x_scales, sf_scale_for_nvfp4, topk_idx, topk_weights, num_tokens_per_rank, num_tokens_per_expert, cached_channel_prefix_matrix, cached_rank_prefix_matrix, recv_topk_idx, recv_topk_weights, recv_x_scales, recv_sf_scale_for_nvfp4}) {
            to.has_value() ? to->record_stream(comm_stream) : void();
            if (allocate_on_comm_stream)
                to.has_value() ? to->record_stream(compute_stream) : void();
        }
...
std::tuple<torch::Tensor, std::optional<torch::Tensor>, torch::Tensor, torch::Tensor, torch::Tensor, std::optional<torch::Tensor>, std::optional<torch::Tensor>, std::optional<EventHandle>, std::optional<std::function<void()>>>
Buffer::low_latency_dispatch(const torch::Tensor& x, const torch::Tensor& topk_idx,
    ...
    if (topk_weights.has_value()) {
        EP_HOST_ASSERT(topk_weights->dim() == 2 and topk_weights->is_contiguous());
        EP_HOST_ASSERT(topk_weights->size(0) == topk_idx.size(0) and topk_weights->size(1) == topk_idx.size(1));
    }
    ...
    auto packed_recv_topk_weights = topk_weights.has_value() ? std::optional<torch::Tensor>(torch::empty({num_local_experts, num_ranks * num_max_dispatch_tokens_per_rank}, torch::dtype(torch::kFloat32).device(torch::kCUDA))) : std::nullopt;
    auto packed_recv_rank_info = topk_weights.has_value() ? std::optional<torch::Tensor>(torch::empty({num_local_experts, num_ranks * num_max_dispatch_tokens_per_rank}, torch::dtype(torch::kInt32).device(torch::kCUDA))) : std::nullopt;
    ...
    auto launcher = [=](int phases) {
        internode_ll::dispatch(packed_recv_x.data_ptr(), packed_recv_x_scales_ptr,
                               ...
                               packed_recv_topk_weights.has_value() ? packed_recv_topk_weights->data_ptr<float>() : nullptr,
                               packed_recv_rank_info.has_value() ? packed_recv_rank_info->data_ptr<int>() : nullptr,
    ...
    // Return values
    return {packed_recv_x, packed_recv_x_scales, packed_recv_count, packed_recv_src_info, packed_recv_layout_range, packed_recv_topk_weights, packed_recv_rank_info, event, recv_hook};
#else
    EP_HOST_ASSERT(false and "NVSHMEM is disabled during compilation");
    return {};
#endif
...
Buffer::dispatch_pcie(const torch::Tensor& x, const std::optional<torch::Tensor>& x_scales,
                      const std::optional<torch::Tensor>& topk_idx, const std::optional<torch::Tensor>& topk_weights,
    ...
    if (previous_event.has_value()) {
        ...
        // 让comm_stream等待compute_stream上的任务完成后再继续执行。
        stream_wait(comm_stream, compute_stream);
    }
...
std::tuple<torch::Tensor, std::optional<torch::Tensor>, std::optional<EventHandle>>
Buffer::combine_pcie(const torch::Tensor& recv_x, const std::optional<torch::Tensor>& recv_topk_weights,
                     const std::optional<torch::Tensor>& bias_0, const std::optional<torch::Tensor>& bias_1,
    ...
    if (bias_0.has_value()) {
        ...
        EP_HOST_ASSERT(bias_0->size(0) == num_combined_tokens and bias_0->size(1) == hidden);
        bias_0_ptr = bias_0->data_ptr();
    }
    if (bias_1.has_value()) {
        EP_HOST_ASSERT(bias_1->dim() == 2 && bias_1->is_contiguous());
        EP_HOST_ASSERT(bias_1->size(0) == num_combined_tokens and bias_1->size(1) == hidden);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    pybind11::class_<deep_ep::Buffer>(m, "Buffer")
        ...
        .def("dispatch_pcie", &deep_ep::Buffer::dispatch_pcie)
        .def("combine_pcie", &deep_ep::Buffer::combine_pcie)
